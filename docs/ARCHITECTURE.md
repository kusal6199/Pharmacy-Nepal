...
Sales and purchase returns are append-only reversals tied to exact original transaction lines. A sales return reloads the original sale and each selected `sale_line` inside one SQLite transaction, rejects a return date before the source sale date, verifies line ownership and the exact original batch, sums prior returns live, replaces draft pricing with the original line's immutable sale-price snapshot, allocates a sales-return number, inserts the return header and lines, and appends one positive-magnitude `SALE_RETURN` movement per line. The stock view interprets that movement as an addition.

A purchase return follows the same transaction shape against `purchase` and `purchase_line`. It rejects a return date before the source purchase date, validates the copied supplier, and reloads current `batch_stock` so the return cannot remove more units than are physically available after intervening sales or returns. It replaces draft cost with the original purchase-line snapshot, allocates a purchase-return number, persists the return, and appends positive-magnitude `PURCHASE_RETURN` movements that the stock view interprets as subtraction.
...
Balances are derived, never stored. Customer balance is opening balance plus `CREDIT` sales and customer-credit payouts, less customer-linked `CREDIT` sales returns and payments received. Supplier balance is opening balance plus `CREDIT` purchases and supplier-credit refunds received, less `CREDIT` purchase returns and payments made. A payout/refund-received has a positive sign because it moves a negative party-credit balance toward zero. The transaction headers themselves are ledger events; only genuine manual opening or settlement events are inserted into `customer_account_entry` or `supplier_account_entry`. Negative balances remain meaningful party credit and are never clamped.
...
Manual openings and settlements use the shared `TransactionRunner`: party existence, current derived balance, opening uniqueness, validation, and insert occur in one transaction. Only Cash or QR may settle a balance. Payment received/payment made cannot exceed a positive outstanding amount; customer-credit payout/supplier-credit refund received requires a negative balance and cannot exceed that credit's magnitude. Account-entry repositories offer insertion only; there is no update/delete path.

The combined credit screen asks the services for entry types appropriate to the selected balance sign. It shows payment actions for positive balances, payout/refund actions for negative balances, and no settlement action at zero; validation below JavaFX remains authoritative. Enum display labels are separate from their stable persisted names, and manual payout/refund ledger entries use the supplied reference or a readable Cash/QR fallback. These financial settlements create no inventory movement and never update their originating sale, sales return, purchase, or purchase return.

Credit sales must be refunded through Credit so an unpaid sale cannot produce a cash/QR payout while leaving Udharo unchanged. For a Credit refund, the effective customer is `COALESCE(sales_return.customer_id, sale.customer_id)`: a customerless Cash/QR sale can therefore issue store credit to an active customer selected on the return, while a sale that already owns a customer continues to use that original link. The return-level foreign key never mutates the immutable source sale. The shared customer selector/create component is used by both POS and Sales Return, while transaction-aware validation rechecks the customer and payment/date invariants before persistence.

This is a party subledger only. It does not introduce a chart of accounts, double-entry journals, cash book, P&L, balance sheet, tax accounting, due dates, aging, interest, credit limits, or any inventory movement/view/type change. Application phase V8 first advances Flyway from schema V5 to V6 through `V6__create_credit_udharo_ledger.sql`; the V8 return-customer follow-up advances schema V6 to V7 through the nullable-column migration `V7__add_sales_return_customer.sql`; and the negative-credit settlement follow-up advances schema V7 to V8 through `V8__add_credit_payout_settlements.sql`.
